...
export default function Members() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://localhost:7101/api/AppUser/SearchUserByEmailorUsername?value=${searchValue}`);
        ...
        const timerId = setTimeout(() => {
            if (searchValue.trim() !== '') { // Eğer arama değeri boş değilse
                fetchData();
            } else {
                                                                    ...
                                                                    searchResult && searchResult.length > 0 && (
                                                                        <div>
                                                                            {searchResult.map((result) => (
                                                                                <Card key={result.id} className='custom-card p-3 mt-1 position-absolute w-100'>
                                                                                    <div className='d-flex align-items-center search-result'>
                                                                                        <img className='rounded-circle me-2' style={{ width: '36px', height: '36px' }} src={result.imageUrl} alt="" />
                                                                                        <div>
                                                                                            <p className='m-0 small'>{result.name}</p>
                                                                                            <p className='m-0 small'>{result.lastLogin ? "Has logged in recently" : "Hasn't logged in recently"}</p>
                                                                                        </div>
                                                                                    </div>
                                                                                </Card>
                                                                            ))}
                                ...
                                <Form.Group className="mb-1" controlId="create-workspace-name">
                                    <div className='d-flex align-items-center'>
                                            ...
                                            {inputResult ? (
                                                searchResult && searchResult.length > 0 && (
                                                    <div>
                                                        {searchResult.map((result) => (
                                                            <Card
                                                                onClick={() => handleCardClick(result.id, result.username)}
                                                                key={result.id} className='custom-card p-3 mt-1 position-absolute w-100'>
                                                                <div className='d-flex align-items-center search-result'>
                                                                    <img className='rounded-circle me-2' style={{ width: '36px', height: '36px' }} src={result.imageUrl} alt="" />
                                                                    <div>
                                                                        <p className='m-0 small'>{result.username}</p>
                                                                        <p className='m-0 small'>{result.email}</p>
                                                                        <p className='m-0 small'>{result.lastLogin ? "Has logged in recently" : "Hasn't logged in recently"}</p>
                                                                    </div>
                                                                </div>
                                                            </Card>
                                        ...
                                        <div className='d-flex align-items-center'>
                                            ...
                                            <Button onClick={workspaceAddUser.handleSubmit} className={Style.modalShareButton}>Add</Button>
                                        </div>
                                    </div>
                                </Form.Group>
                            </Form>
